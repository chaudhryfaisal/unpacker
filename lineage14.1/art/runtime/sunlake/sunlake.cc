...
mirror::ClassLoader* Sunlake::getAppClassLoader() {
  Thread* const self = Thread::Current();
...
size_t Sunlake::getCodeItemSize(ArtMethod* method) {
...
void Sunlake::dumpMethod(ArtMethod *method, int nop_size) {
  ...
  const DexFile::CodeItem* code_item = method->GetCodeItem();
  uint32_t code_item_size = (uint32_t)Sunlake::getCodeItemSize(method);
